"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
var _operations = require("./operations/index.js");
Object.keys(_operations).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _operations[key]) return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function () {
      return _operations[key];
    }
  });
});
var _backupService = require("./backupService/index.js");
Object.keys(_backupService).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _backupService[key]) return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function () {
      return _backupService[key];
    }
  });
});
var _bip = require("./bip39/index.js");
Object.keys(_bip).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _bip[key]) return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function () {
      return _bip[key];
    }
  });
});
var _addressService = require("./addressService/index.js");
Object.keys(_addressService).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _addressService[key]) return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function () {
      return _addressService[key];
    }
  });
});
var _locale = require("./locale/index.js");
Object.keys(_locale).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _locale[key]) return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function () {
      return _locale[key];
    }
  });
});
//# sourceMappingURL=index.js.map