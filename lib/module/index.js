...
const TangemSdkCodoraReactNative = NativeModules.TangemSdkCodoraReactNative ? NativeModules.TangemSdkCodoraReactNative : new Proxy({}, {
  get() {
    throw new Error(LINKING_ERROR);
  }
});
export function scan(cardId, accessCode) {
  return TangemSdkCodoraReactNative.scan(cardId, accessCode);
}
export function sign(unsignedHex, pubKeyBase58, cardId, accessCode) {
